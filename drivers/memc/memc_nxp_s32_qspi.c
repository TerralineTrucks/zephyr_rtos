...
static int memc_nxp_s32_qspi_init(const struct device *dev)
{
	...
	if (status != STATUS_QSPI_IP_SUCCESS) {
		LOG_ERR("Fail to initialize QSPI controller %d (%d)", data->instance, status);
		return -EIO;
	}
...
#define QSPI_DATA_CFG(n)                                                                           \
	IF_ENABLED(FEATURE_QSPI_DDR,                                                               \
		   (.dataRate =                                                                    \
			    _CONCAT(QSPI_IP_DATA_RATE_, DT_INST_STRING_UPPER_TOKEN(n, data_rate)), \
...
#define QSPI_ADDR_CFG(n)                                                                           \
	IF_ENABLED(FEATURE_QSPI_ADDR_CFG, (.columnAddr = DT_INST_PROP_OR(n, column_space, 0),      \
					   .wordAddresable = DT_INST_PROP(n, word_addressable), ))

#define QSPI_BYTES_SWAP_ADDR(n)                                                                    \
	IF_ENABLED(FEATURE_QSPI_BYTES_SWAP_ADDR, (.byteSwap = DT_INST_PROP(n, byte_swapping), ))

#define QSPI_SAMPLE_DELAY(n)                                                                       \
	COND_CODE_1(DT_INST_PROP(n, sample_delay_half_cycle),                                      \
		    (QSPI_IP_SAMPLE_DELAY_HALFCYCLE_EARLY_DQS), (QSPI_IP_SAMPLE_DELAY_SAME_DQS))

#define QSPI_SAMPLE_PHASE(n)                                                                       \
	COND_CODE_1(DT_INST_PROP(n, sample_phase_inverted), (QSPI_IP_SAMPLE_PHASE_INVERTED),       \
...
#define QSPI_SFP_CFG(n)                                                                            \
	{                                                                                          \
		.MasterTimeout = 0xffffffffUL,                                                     \
		.Tg =                                                                              \
			{                                                                          \
				{.SecureAttribute = 0b11,                                          \
				 .MaskType = 1,                                                    \
				 .Valid = 1,                                                       \
				 .Mask = 0b111111,                                                 \
				 .DomainId = 0b111111},                                            \
		...
		.Frad =                                                                            \
			{                                                                          \
				{                                                                  \
					...
					.Md0Acp = 0b110,                                           \
					.Md1Acp = 0b110,                                           \
					.ExclusiveAccessLock = 0b00,                               \
					.ExclusiveAccessOwner = 0b000000,                          \
				},                                                                 \
				{},                                                                \
				{},                                                                \
				{},                                                                \
...
#define MEMC_NXP_S32_QSPI_CONTROLLER_CONFIG(n)                                                     \
	...
	BUILD_ASSERT(_CONCAT(FEATURE_QSPI_, DT_INST_STRING_UPPER_TOKEN(n, a_rx_clock_source)) ==   \
		     ...
		     "a-rx-clock-source source mode selected is not supported");                   \
                                                                                                   \
	static const Qspi_Ip_ControllerConfigType memc_nxp_s32_qspi_controller_cfg_##n = {         \
		.csHoldTime = DT_INST_PROP(n, cs_hold_time),                                       \
		.csSetupTime = DT_INST_PROP(n, cs_setup_time),                                     \
		.sampleDelay = QSPI_SAMPLE_DELAY(n),                                               \
		.samplePhase = QSPI_SAMPLE_PHASE(n),                                               \
		.ahbConfig = QSPI_AHB_BUFFERS(n),                                                  \
		.SfpCfg = QSPI_SFP_CFG(n),                                                         \
		QSPI_SIDE_CFG(n, a, A) QSPI_DATA_CFG(n) QSPI_ADDR_CFG(n) QSPI_BYTES_SWAP_ADDR(n)}
