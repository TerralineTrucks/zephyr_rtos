...
static const Qspi_Ip_InstrOpType nxp_s32_qspi_lut[][QSPI_LUT_ENTRY_SIZE] = {
	[QSPI_SEQ_RDSR] =
		{
			...
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_STOP, QSPI_IP_LUT_PADS_1,
				    QSPI_IP_LUT_SEQ_END),
		},
	...
	[QSPI_SEQ_SE] =
		{
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_CMD, QSPI_IP_LUT_PADS_1, SPI_NOR_CMD_SE_4B),
	...
	[QSPI_SEQ_BE] =
		{
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_CMD, QSPI_IP_LUT_PADS_1, SPI_NOR_CMD_BE_4B),
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_ADDR, QSPI_IP_LUT_PADS_1, 32U),
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_STOP, QSPI_IP_LUT_SEQ_END,
				    QSPI_IP_LUT_SEQ_END),
		},
	...
	[QSPI_SEQ_RDID] =
		{
			...
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_READ, QSPI_IP_LUT_PADS_1,
				    JESD216_READ_ID_LEN),
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_STOP, QSPI_IP_LUT_SEQ_END,
				    QSPI_IP_LUT_SEQ_END),
		},
	...
	[QSPI_SEQ_READ_1_2_2] =
		{
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_CMD, QSPI_IP_LUT_PADS_1,
				    SPI_NOR_CMD_2READ_4B),
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_ADDR, QSPI_IP_LUT_PADS_2, 32U),
	...
	[QSPI_SEQ_PP_1_1_1] =
		{
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_CMD, QSPI_IP_LUT_PADS_1, SPI_NOR_CMD_PP_4B),
			QSPI_LUT_OP(QSPI_IP_LUT_INSTR_ADDR, QSPI_IP_LUT_PADS_1, 32U),
...
#if !defined(CONFIG_FLASH_NXP_S32_QSPI_NOR_SFDP_RUNTIME)
static int nxp_s32_qspi_read_status_register(const struct device *dev, uint8_t reg_num,
					     uint8_t *val)
{
	struct nxp_s32_qspi_data *data = dev->data;
	uint16_t lut_idx;
...
static int nxp_s32_qspi_write_status_register(const struct device *dev, uint8_t reg_num,
					      uint8_t val)
{
	...
	if (status == STATUS_QSPI_IP_SUCCESS) {
		...
		LOG_ERR("IPSERROR 0x%08x, ERRSTAT 0x%08x, FSMSTAT 0x%08x", base_addr->IPSERROR,
			base_addr->ERRSTAT, base_addr->FSMSTAT);
		ret = -EIO;
	}
...
static int nxp_s32_qspi_sfdp_read(const struct device *dev, off_t offset, void *buf, size_t len)
{
	...
	status = Qspi_Ip_RunReadCommand(data->instance, data->read_sfdp_lut_idx, (uint32_t)offset,
					(uint8_t *)buf, NULL, (uint32_t)len);
	if (status != STATUS_QSPI_IP_SUCCESS) {
...
#define QSPI_STATUS_REG_CFG(n)                                                                     \
	{                                                                                          \
		...
		.writeEnableSRLut = QSPI_LUT_IDX(QSPI_SEQ_REG_WREN),                               \
...
#define FLASH_NXP_S32_QSPI_INIT_DEVICE(n)                                                          \
	...
	static const struct nxp_s32_qspi_config nxp_s32_qspi_config_##n = {                        \
		...
		IF_ENABLED(CONFIG_FLASH_PAGE_LAYOUT, (QSPI_PAGE_LAYOUT(n), )) COND_CODE_1(         \
			CONFIG_FLASH_NXP_S32_QSPI_NOR_SFDP_RUNTIME, (),                            \
			(.memory_cfg = QSPI_MEMORY_CFG(n), .qer_type = QSPI_QER_TYPE(n),           \
